...
import websiteService, { Objective } from '../services/website.service';

function WebsiteObjective() {
  ...
  const [editingContent, setEditingContent] = useState<Partial<Objective> | null>(null);
  ...
  const loadData = async () => {
    try {
      const content = await websiteService.getObjectives();
  ...
  const handleSaveContent = async () => {
    ...
    try {
      if (editingContent.id) {
        await websiteService.updateObjective(editingContent.id, editingContent);
      } else {
        await websiteService.createObjective(editingContent);
      }
  ...
  const handleDeleteContent = async (id: string) => {
    ...
    try {
      await websiteService.deleteObjective(id);
      showSnackbar('Content deleted', 'success');
      setObjectives(objectives.filter(c => c.id !== id));
    } catch {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f5f5f5' }}>
      ...
      <Box sx={{ p: 3 }}>
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <Button variant="contained" startIcon={<Add />} onClick={() => { setEditingContent({ description: '', display_order: objectives.length }); setContentDialog(true); }}>Add Objective</Button>
          </Box>
          <Alert severity="info" sx={{ mb: 2 }}>Add objective description. Supports alphanumeric, special characters, and hyperlinks.</Alert>
          ...
          {objectives.length === 0 && (
            <Card sx={{ p: 4, textAlign: 'center' }}>
              <Flag sx={{ fontSize: 48, color: 'grey.400', mb: 2 }} />
              <Typography color="text.secondary">No objectives added yet.</Typography>
            </Card>
          )}
        </Box>
      </Box>
      ...
      <Dialog open={contentDialog} onClose={() => setContentDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>{editingContent?.id ? 'Edit Objective' : 'Add Objective'}</DialogTitle>
        <DialogContent>
          <TextField 
            fullWidth 
            label="Description *" 
...
export default WebsiteObjective;
