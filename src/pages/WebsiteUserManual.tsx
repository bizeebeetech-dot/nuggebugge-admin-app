...
import websiteService, { UserManual } from '../services/website.service';

function WebsiteUserManual() {
  ...
  const loadData = async () => {
    try {
      const content = await websiteService.getUserManuals();
      setItems(content);
    } catch {
  ...
  const handleSaveContent = async () => {
    ...
    try {
      if (editingContent.id) {
        await websiteService.updateUserManual(editingContent.id, editingContent);
      } else {
        await websiteService.createUserManual(editingContent);
      }
  ...
  const handleDeleteContent = async (id: string) => {
    ...
    try {
      await websiteService.deleteUserManual(id);
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f5f5f5' }}>
        ...
        <Box sx={{ mb: 4 }}>
          ...
          {items.length === 0 && (
            <Card sx={{ p: 4, textAlign: 'center' }}>
              ...
              <Typography color="text.secondary">No user manuals added yet.</Typography>
            </Card>
          )}
        </Box>
      </Box>
      ...
      <Dialog open={contentDialog} onClose={() => setContentDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>{editingContent?.id ? 'Edit User Manual' : 'Add User Manual'}</DialogTitle>
        <DialogContent>
          <TextField 
            fullWidth 
            label="Description *" 
...
export default WebsiteUserManual;
