...
import websiteService, { GovtProjects } from '../services/website.service';

function WebsiteGovtProjects() {
  ...
  const [items, setItems] = useState<GovtProjects[]>([]);
  const [contentDialog, setContentDialog] = useState(false);
  const [editingContent, setEditingContent] = useState<Partial<GovtProjects> | null>(null);
  ...
  const loadData = async () => {
    try {
      const content = await websiteService.getGovtProjects();
      setItems(content);
    } catch {
  ...
  const handleSaveContent = async () => {
    ...
    try {
      if (editingContent.id) {
        await websiteService.updateGovtProjects(editingContent.id, editingContent);
      } else {
        await websiteService.createGovtProjects(editingContent);
      }
  ...
  const handleDeleteContent = async (id: string) => {
    ...
    try {
      await websiteService.deleteGovtProjects(id);
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f5f5f5' }}>
      ...
      <Box sx={{ p: 3 }}>
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <Button variant="contained" startIcon={<Add />} onClick={() => { setEditingContent({ description: '', display_order: items.length }); setContentDialog(true); }}>Add Govt. Project</Button>
          </Box>
          <Alert severity="info" sx={{ mb: 2 }}>Add government projects description. Supports alphanumeric, special characters, and hyperlinks.</Alert>
          ...
          {items.length === 0 && (
            <Card sx={{ p: 4, textAlign: 'center' }}>
              ...
              <Typography color="text.secondary">No government projects added yet.</Typography>
            </Card>
          )}
        </Box>
      </Box>
      ...
      <Dialog open={contentDialog} onClose={() => setContentDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>{editingContent?.id ? 'Edit Govt. Project' : 'Add Govt. Project'}</DialogTitle>
        <DialogContent>
          <TextField 
            fullWidth 
            label="Description *" 
...
export default WebsiteGovtProjects;
